...
class NPUModelRunner(GPUModelRunner):
    ...
    def execute_model(
        ...
        logger.info("[PCP-WORKER] execute_model START pcp=%d tokens=%d",
                    self.pcp_rank, scheduler_output.total_num_scheduled_tokens)
        ...
        with record_function_or_nullcontext("prepare input"):
            with self.synchronize_input_prep():
                ...
                logger.info("[PCP-WORKER] before _update_states pcp=%d", self.pcp_rank)
                deferred_state_corrections_fn = self._update_states(
                    scheduler_output
                )
                logger.info("[PCP-WORKER] after _update_states pcp=%d", self.pcp_rank)
                ...
                logger.info("[PCP-WORKER] before _prepare_inputs pcp=%d", self.pcp_rank)
                (
                    logits_indices,
                    spec_decode_metadata,
                    total_num_scheduled_tokens,
                ...
                logger.info("[PCP-WORKER] after _prepare_inputs pcp=%d", self.pcp_rank)
                ...
                logger.info("[PCP-WORKER] before _build_attention_metadata pcp=%d", self.pcp_rank)
                ...
                logger.info("[PCP-WORKER] after _build_attention_metadata pcp=%d", self.pcp_rank)
        ...
        with (
            ...
            logger.info("[PCP-WORKER] before _model_forward pcp=%d", self.pcp_rank)
            hidden_states = self._model_forward(
                num_tokens_padded, input_ids, positions, intermediate_tensors, inputs_embeds, **model_kwargs
            )
            logger.info("[PCP-WORKER] after _model_forward pcp=%d", self.pcp_rank)
        with record_function_or_nullcontext("post process"):
            ...
            if self.pcp_size > 1:
                ...
                logger.info("[PCP-WORKER] before get_restore_hidden_states pcp=%d", self.pcp_rank)
                hidden_states = self.pcp_manager.get_restore_hidden_states(hidden_states)
                logger.info("[PCP-WORKER] after get_restore_hidden_states pcp=%d", self.pcp_rank)
            ...
            if not self.broadcast_pp_output:
                ...
                logger.info("[PCP-WORKER] before compute_logits pcp=%d", self.pcp_rank)
                sample_hidden_states = hidden_states[logits_indices]
                logits = self.model.compute_logits(sample_hidden_states)
                logger.info("[PCP-WORKER] after compute_logits pcp=%d", self.pcp_rank)
            else:
                # Rare case.
