...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    def __init__(
        ...
        **kwargs,
        ...
        self._pcp_ag_seq = 0
        if not hasattr(AscendAttentionCPBackend, '_global_pcp_seq'):
            AscendAttentionCPBackend._global_pcp_seq = 0
    ...
    def _prefill_query_all_gather(self, attn_metadata, prefill_query):
        _lid = id(self) % 10000
        if self.pcp_size > 1:
            AscendAttentionCPBackend._global_pcp_seq += 1
            _gseq = AscendAttentionCPBackend._global_pcp_seq
            print(f"[QAG-DEBUG] pcp={self.pcp_rank} L={_lid} g={_gseq} before q_allgather", flush=True)
            prefill_query = get_pcp_group().all_gather(prefill_query, 0)
            print(f"[QAG-DEBUG] pcp={self.pcp_rank} L={_lid} g={_gseq} after q_allgather", flush=True)
    ...
    def _compute_prefill_context(
        ...
        actual_kv_cumsum = prefill_metadata.chunked_context.actual_seq_lengths_kv
        actual_q_cumsum = attn_metadata.prefill.chunked_context.actual_chunk_seq_lengths

        per_req_kv_lens = [actual_kv_cumsum[0]] + [
            actual_kv_cumsum[i] - actual_kv_cumsum[i - 1] for i in range(1, len(actual_kv_cumsum))
        ]
        ...
        if not has_zero_kv:
            prefix_chunk_output, prefix_chunk_lse = torch.ops.npu.npu_fused_infer_attention_score(
                ...
                antiquant_scale=None,
                softmax_lse_flag=True,
                actual_seq_lengths_kv=actual_kv_cumsum,
                actual_seq_lengths=actual_q_cumsum,
            )
            return prefix_chunk_output, prefix_chunk_lse

        # Mixed batch: some requests have 0 context KV (new requests batched
        # with running requests that have existing KV). The NPU kernel cannot
        # handle query>0 with KV=0, so we filter those out, call the kernel
        # only for valid requests, and fill zeros/-inf for the rest.
        if isinstance(actual_q_cumsum, torch.Tensor):
            actual_q_list = actual_q_cumsum.tolist()
        else:
            actual_q_list = list(actual_q_cumsum)
        per_req_q_lens = [int(actual_q_list[0])] + [
            int(actual_q_list[i] - actual_q_list[i - 1]) for i in range(1, len(actual_q_list))
        ]

        full_output = torch.zeros(
            query.size(0), num_heads, self.head_size, dtype=query.dtype, device=query.device
        )
        full_lse = torch.full(
            (query.size(0), num_heads, 1), fill_value=-torch.inf, dtype=torch.float32, device=query.device
        )
        ...
        q_off, kv_off = 0, 0
        for ql, kvl in zip(per_req_q_lens, per_req_kv_lens):
            if kvl > 0:
                valid_queries.append(query[q_off : q_off + ql])
                valid_keys.append(key[kv_off : kv_off + kvl])
                valid_values.append(value[kv_off : kv_off + kvl])
                valid_q_lens.append(ql)
                valid_kv_lens.append(kvl)
                q_slices.append((q_off, q_off + ql))
            q_off += ql
            kv_off += kvl

        if valid_queries:
            cat_q = torch.cat(valid_queries, dim=0)
            cat_k = torch.cat(valid_keys, dim=0)
            cat_v = torch.cat(valid_values, dim=0)
            filt_kv_cumsum = torch.cumsum(
            ...
            chunk_out, chunk_lse = torch.ops.npu.npu_fused_infer_attention_score(
                ...
                antiquant_scale=None,
                softmax_lse_flag=True,
                actual_seq_lengths_kv=filt_kv_cumsum,
                actual_seq_lengths=filt_q_cumsum,
            )

            out_off = 0
            for (q_start, q_end), ql in zip(q_slices, valid_q_lens):
                full_output[q_start:q_end] = chunk_out[out_off : out_off + ql]
                full_lse[q_start:q_end] = chunk_lse[out_off : out_off + ql]
                out_off += ql
    ...
    def _gather_global_context_output(self, local_context_attn_output):
        ...
        if self.pcp_size > 1:
            AscendAttentionCPBackend._global_pcp_seq += 1
            _gseq = AscendAttentionCPBackend._global_pcp_seq
            print(f"[GATHER-DEBUG] pcp={self.pcp_rank} L={_lid} g={_gseq} before pcp_allgather shape={dcp_context_attn_output.shape}", flush=True)
            global_context_attn_output = get_pcp_group().all_gather(dcp_context_attn_output, dim=-1)
            print(f"[GATHER-DEBUG] pcp={self.pcp_rank} L={_lid} g={_gseq} after pcp_allgather", flush=True)
        else:
